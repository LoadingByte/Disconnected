...
public class VulnSource implements NamedValue {
     ...
     *        Such actions describe what an attacked computer part (e.g. a program) should do.
     */
    public VulnSource(String name, String usage, int weight, List<Action> actions) {
     ...
     * <br>
     * The usage string is used by the {@link VulnSourceRegistry#getValuesByUsage(String...)} method to retrieve vulnerability sources
     ...
     * Such actions describe what an attacked computer part (e.g. a program) should do.
     * When a new vulnerability is generated, a subset of these actions is selected based on their {@link Action#getVulnProbability() vulnProbability} values.<br>
     * <br>
     ...
     * <br>
     * Note that the {@link Action#getAttackWeight() attack weight}s are used to select which action is actually used for an attack later on.
     *
     ...
     * On the other hand, an SQL injection vulnerability could be used to access a database.<br>
     * <br>
     * When a new vulnerability is generated, a subset of all defined actions is selected based on their {@link #getVulnProbability() vulnProbability} values.
     ...
     * <br>
     * When a new attack (it might also be an exploit) is "generated" from a vulnerability, the {@link #getAttackWeight() attack weight}s ({@link #getAttackWeight()}) are
     * used to determine which available action is used for the attack.
    ...
    public static class Action implements NamedValue {
        ...
        /**
         * Creates a new vulnerability action data object (should only be used inside a {@link VulnSource vulnerability source} object.
         *
         * @param name The name of the new action.
         * @param vulnProbability The vulnerability probability (must be &gt;= 0 and &lt;= 1).
         *        See {@link #getVulnProbability()} for more details.
         * @param attackWeight The attack weight of the new action.
         *        See {@link #getAttackWeight()} for more details.
         */
        public Action(String name, float vulnProbability, int attackWeight) {

            Validate.notNull(name, "Vulnerability source action cannot be null");
            Validate.inclusiveBetween(0F, 1F, vulnProbability, "Vulnerability probability (chance) must be >= 0 and <= 1: %d", vulnProbability);
            Validate.isTrue(attackWeight >= 0, "Vulnerability source action attack weight must be >= 0: %d", attackWeight);

            this.name = name;
            this.vulnProbability = vulnProbability;
         ...
         */
        public float getVulnProbability() {

            return vulnProbability;
        }
