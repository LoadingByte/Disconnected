...
public class ConfigureNamedTypeCommand extends ConfigureNamedValueCommand<Mapping<String, Class<?>>> {

    public ConfigureNamedTypeCommand(MapRegistry<String, Class<?>> registry) {

        super("named type", registry);
    }
    ...
    protected Mapping<String, Class<?>> changeValue(Document config, Element commandElement, Mapping<String, Class<?>> oldValue) throws JDOMException {
        ...
        if (typeElement != null) {
            ...
            type = ParserUtils.parseClass(config, "named type class for '" + name + "'", File.class, typeString, type);
        }
