...
public class RemoveWorldInitializerMappingCommand implements ConfigCommandParser {
    ...
    public void parse(Document config, Element commandElement) throws JDOMException {
        ...
        Mapping<Class<? extends FeatureHolder>, FeatureDefinition<?>> mapping = AddWorldInitializerMappingCommand.parseMapping(config, commandElement);

        if (mapping != null) {
            registry.removeValue(mapping);
        }
    }
