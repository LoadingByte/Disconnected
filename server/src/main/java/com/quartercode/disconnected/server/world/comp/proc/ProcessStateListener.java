 ...
 * Such listeners can be activated by adding the to a process using {@link Process#addStateListener(ProcessStateListener)}.<br>
 * <br>
 * Note that the process might still need to execute some actions after all state listeners have been called, even if the state value has already been changed.
 * For example, on process stopping, the state listeners are called <i>before</i> the process itself is unlinked from the process tree.
 * Therefore, the parent process of the stopped can still be accessed via {@link Process#getSingleParent()}.
 */
